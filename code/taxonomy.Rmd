...
#counts
All_taxa_counts <- read.csv("C:/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rank/All_rank_filt.csv", row.names = 1)

dim(All_taxa_counts) #3399 obs.
...
#metafile
metafile_taxa <- read.csv("C:/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/metafile_taxa.csv", sep=",", row.names=1)
...
#taxonomy
All_taxa <- read.csv("C:/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rank/All_rank_samples.taxa.csv", row.names = 1)
...
#saveRDS(physeq_taxonomy, "C:/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/physeq_taxonomy.rds")

#saveRDS(physeq_taxonomy_relab, "C:/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/physeq_taxonomy_relab.rds")

```

# upload phyloseq obj

```{r phyloseq upload, message=FALSE, warning=FALSE}

physeq_taxonomy <- readRDS("C:/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/physeq_taxonomy.rds")

physeq_taxonomy_relab <- readRDS("C:/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/physeq_taxonomy_relab.rds")
...
print(pcoa_tax)
ggsave("/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/figure_paper/pcoa_taxa.jpeg", width = 20, height = 15, units = "cm")
...
ggsave("/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/figure_paper/pcoa_taxa_env.jpeg", width = 20, height = 15, units = "cm")
...
permanova_tax
write.csv(permanova_tax$aov.tab, "/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/perm_taxa_site.csv")

pairwise_tax <- pairwise.adonis2(bray_taxa~country,data=meta.bray_taxa, p.adjust.m = "bonferroni")

write.csv(pairwise_tax$DE_vs_ES,"/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/posthoc_tax_DEvsES.csv")

write.csv(pairwise_tax$DE_vs_SE, "/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/posthoc_tax_DEvsSE.csv")

write.csv(pairwise_tax$ES_vs_SE, "/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/posthoc_tax_ESvsSE.csv")
...
```
***No drought effect also when accounting for batch correction***
...
print(p_bact_phylum)
ggsave("/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/figure_paper/p_bact_phylum.jpeg", width = 20, height = 10, units = "cm")
...
#ggsave("/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/p_bact_genus.jpeg", width = 30, height = 15, units = "cm")
...
#how many different species detected in the system

#all 
taxa_richness <- estimate_richness(physeq_taxonomy)
plot_richness(physeq_taxonomy, x="country", color="treatment", measures=c("Observed", "Shannon")) + geom_boxplot() + stat_boxplot(geom = "errorbar") + labs(color="Drought treatment")

#only bacteria
taxa_richness_Bacteria <- estimate_richness(taxa_Bacteria)

plot_richness(taxa_Bacteria, x="country", color="treatment", measures=c("Observed", "Shannon")) + geom_boxplot() + stat_boxplot(geom = "errorbar") + labs(color="Drought treatment")
...
fun_tax <- read.table("C:/Users/Katja/OneDrive - Lund University/Skrivbordet/FunctionalGenes/Enzfam_tax_count_new.tsv", sep = "\t", header = T)
dim(fun_tax) #25290 98
...
#saveRDS(fun_samples_long, "C:/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/taxa_long_format.rds")
...
#fun_samples_long <- readRDS("C:/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/taxonomy_contigtax/rds/taxa_long_format.rds")
...
#ggsave("/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/taxa_function.jpeg", width = 20, height = 15, units = "cm")

```
 
### genus DE
 
```{r fun-taxa genus DE, message=FALSE, warning=FALSE}

taxa_fun_DE <- fun_samples_long %>% filter(countryID == "DE")

genus_DE <- taxa_fun_DE %>% 
    group_by(FuncID,Genus, sampleID, treatment) %>%
    summarise(sum_counts=sum(rel_abund)) %>%
    group_by(FuncID, Genus,treatment) %>%
    summarise(mean_relab=mean(sum_counts))
nrow(genus_DE)

filt_genus_DE <- taxa_fun_DE %>% 
   group_by(FuncID,Genus,sampleID,treatment) %>%
   summarise(sum_counts=sum(rel_abund)) %>%  
   group_by(FuncID, Genus) %>%
   summarise(mean_relab=mean(sum_counts)) %>%
   group_by(Genus) %>%
   summarise(sum_counts=sum(mean_relab)) %>%
   filter(sum_counts>0.01)  #to discard genus below 1%
...
filt_func_genus_DE <- genus_DE %>%
   group_by(FuncID,treatment) %>%
   summarise(sum_counts=sum(mean_relab)) %>%
   filter(sum_counts>0.01)

final_genus_DE <- genus_DE %>%
  mutate(Genus = ifelse(Genus %in% filt_genus_DE$Genus, Genus, "Others")) %>%
  group_by(FuncID, Genus, treatment) %>%
  summarise(final_counts=sum(mean_relab)) %>%
  filter(FuncID %in% filt_func_genus_DE$FuncID)

colors <- c("#46ACC8", "#CEAB07", "#35274A", "#79402E", "#FD6467","#02401B", "#9986A5","#DD8D29", "#EAD3BF", 
            "#8D8680","#81A88D", "#899DA4", "#D69C4E", "#F4B5BD", "#AA9486", "#FAEFD1")

final_genus_DE$Genus <- factor(final_genus_DE$Genus, levels = c("Actinoplanes", "Amycolatopsis", "Bradyrhizobium", "Luteitalea", "Micromonospora", "Mycobacterium", "Mycolicibacterium", "Nocardioides", "Pseudomonas", "Streptomyces", "Xanthomonas", "Unclassified.Actinobacteria", "Unclassified.Bacteroidetes", "Unclassified.Proteobacteria", "Unclassified", "Others"))

final_genus_DE$treatment <- factor(final_genus_DE$treatment, levels = c("R", "RC", "C"))
                                      

ggplot(final_genus_DE, aes(x=fct_rev(FuncID), y=final_counts, fill=Genus)) + 
  geom_col(position = "identity") + xlab("CAZy enzyme family") +
  ylab("Relative abundance (%)") + coord_flip() + facet_grid(~treatment) + scale_fill_manual(values=colors) +
  scale_y_continuous(labels = scales::percent)  + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) 
...
ggsave("/Users/Katja/Box/PhD Lund Katja/Sequencing/Functional_genes/Diamond/data_TDB/relaxed/results/figure_paper/taxa_function_phylum.jpeg", width = 20, height = 10, units = "cm")
